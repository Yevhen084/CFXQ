...
const PhoneVerify = ({ route, navigation } : AuthenticationScreenProps<'Ver'>) => {
  ...
  const onPressConfirm = async () => {
    ...
    try {
      const result = await axios.post(
        `${API_URL}/Investors/onboarding/confirm`,
      )
    } catch(error) {
  ...
  const onPressResend = async () => {
    try {
      const result = await axios.post(
        `${API_URL}/Investors/onboarding/resend-code`
      )
  ...
  return (
    <View style={styles.content}>
      ...
      <View style={[Layout.center, Gutters.regularPadding]}>
        <Text weight='bold' size='medium'>Verify Email</Text>
        <Text align='center' mt={20} pl={15} pr={15}>Email verification code has been sent to your email address.</Text>
        <Text align='center' pl={15} pr={15} mt={5}>Please check your inbox and confirm your email.</Text>
      </View>
      ...
      <View style={[Layout.fullWidth, Gutters.regularTMargin]}>
        <Button title="Confirm" style={[Gutters.largeTMargin]} onPress={onPressConfirm}/>
        <Button 
