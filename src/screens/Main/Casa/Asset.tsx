...
const Asset = ({ route }: CasaStackScreenProps) => {
  ...
  const { user, tokenTypes } = useAppSelector(state => state.app);
      ...
      >
        <View>
          ...
          <Text size="tiny" mt={8} color="textGray200">
            {moment(item.paymentDate).format('YYYY, MMM.dd')}
          </Text>
        </View>
      ...
      >
        ...
        >
          <View style={[Layout.row, Layout.alignItemsCenter]}>
            <Image
              source={
                Images.coins[asset?.tokenType.shortName.toLowerCase() ?? 'btc']
              }
          ...
          <View style={[Layout.alignItemsEnd]}>
            <Text weight="medium">{asset?.total}</Text>
        ...
        <View>
          ...
          >
            <Text color="textGray200">Disponibile</Text>
            <Text>{asset?.totalPrincipal}</Text>
          </View>
          <View
            style={[
              Layout.row,
          ...
          >
            <Text color="textGray200">Congela</Text>
            <Text>{0}</Text>
          </View>
        </View>
      </View>
